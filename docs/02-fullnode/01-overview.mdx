...
# Realio Network full node overview
...
In order to run a full node, you need to build `realio-networkd` which requires `Go 1.23+`, `git`, `gcc` and `make` installed.
...
```bash
# Install software using Homebrew (Go 1.23+ required)
brew install go@1.23 git curl jq
...
Once you have set up your environment correctly, you are now ready to install the Realio Network software and start your full
node.
