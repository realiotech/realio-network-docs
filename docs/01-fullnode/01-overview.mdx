...
# Realio Network fullnode overview
...
Inside Realio Network, there are various types of pruning strategies that can be applied. The main ones are:
...
In order to run a fullnode, you need to build `realio-networkd` which requires `Go`, `git`, `gcc` and `make` installed.
